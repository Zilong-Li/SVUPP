...
For benchmarking studies, It is important to evaluate the results by stratifying the SV complexity and the call rate, which is controled by the GQ thresholding. You can achieve this easily by using the [[https://github.com/Zilong-Li/vcfppR?tab=readme-ov-file#vcfcomp-compare-two-vcf-files-and-report-concordance][vcfppR]] (version >= 0.8.2) package.
