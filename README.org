...
SVUPP supports /Docker/, /Singularity/ and /Conda/ containers technology. Therefore, you can choose to use one of the 3 profiles in the =nextflow.config= namely *'docker','singularity' and 'conda'*. *NB*, if you choose to use either /singularity/ or /docker/ profile, you have to set the =params.container= to the local image name. If you use conda profile, you should activate a conda (base) environment first before running SVUPP. Also, if conda is chosen, it may take a while to resolve the conda environment for the first time depending on the conda version and internet connection.
...
All output files are saved in the folder that you specified when running Nextflow command with defaults to *results*. Here is the details:

| Genotyped VCF:      | results/cuteSV2/id.vcf.gz            |
